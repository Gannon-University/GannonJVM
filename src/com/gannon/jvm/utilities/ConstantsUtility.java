package com.gannon.jvm.utilities;

public class ConstantsUtility {
	public final static int INIT_PROGRAM_LINE_NUMBER=1;
	public static final String NEW_LINE = "\n";
	public static final String GANNON_JVM_PATH_TXT = "gannonJVMPath.txt";
	public static final int END_CFG_NODE_ID = Integer.MAX_VALUE;
	public static final int START_CFG_NODE_ID = Integer.MIN_VALUE;
	public static final int METHOD_LEVEL = 2;
	public static final int TESTPATH_LEVEL = 3; 
	
	public static final int UNDEFINED_EXPECTED_VALUE=-1;
	public static final int EXPECTED_TRUE=1;
	public static final int EXPECTED_FALSE=0;
	public static final int RANGE_MAX = 100;
	public static final int UNDEFINED_VAR_ID=99999;
	public static final int UNDEFINED_INDEX_ID = 99999;
	public static final int RETURN_VAR_ID = 888888; 
}
