...
import com.gannon.jvm.path.infeasible.CyclePath;
import com.gannon.jvm.path.infeasible.FEdge;
import com.gannon.jvm.path.infeasible.PredicateRelationGraph;
...
public class TestPath {
	...
	private Input inputs = new Input(1,null,null);
	...
	public boolean isInfeasible(){
		PredicateRelationGraph prg=new PredicateRelationGraph(bClass, bMethod,this);
		List<FEdge> selfLoopEdges=prg.computingSelfLoopEdges();
		for(FEdge e:selfLoopEdges){
			if(e.isConflict()){
				System.out.println("Self loop conflicts!"	);
				return true;
			}
		}
		
		List<CyclePath> cs=prg.computingCycles();
		for(CyclePath g:cs){
			if(g.isAllMarkedAsUsed() && g.isConflict()){
				return true;
			}
		}
		return false;
